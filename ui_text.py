...
UI_TEXT: Dict[str, Dict[str, str]] = {
    "en": {
        ...
        "page.reports.title": "Reports and Findings",
        "page.reports.subtitle": "Scanner-generated vulnerability reports, imported findings, and pivots into Event Console for vulnerable assets and exposed services.",
...
for _lang in ("en", "ru"):
    UI_TEXT[_lang].setdefault("nav.reports", "Reports")
    UI_TEXT[_lang].setdefault("nav.reports.meta", "Vuln")
    UI_TEXT[_lang].setdefault("page.reports.title", "Reports and Findings")
    UI_TEXT[_lang].setdefault(
        "page.reports.subtitle",
        "Scanner-generated vulnerability reports, imported findings, and pivots into Event Console for vulnerable assets and exposed services.",
    )

UI_TEXT["ru"].update(
    {
        "brand.title": "Rdegon SIEM",
        "brand.subtitle": "Корпоративная консоль расследований для телеметрии, детектов, инцидентов и эксплуатации платформы.",
        "nav.monitor": "Мониторинг",
        "nav.content": "Контент и управление",
        "nav.dashboards": "Дашборды",
        "nav.dashboards.meta": "Обзор",
        "nav.incidents": "Инциденты",
        "nav.incidents.meta": "Очередь",
        "nav.events": "События",
        ...
        "shell.show": "Показать",
        "shell.live": "ClickHouse",
        "shell.live.ok": "онлайн",
        "shell.live.down": "недоступен",
        "shell.sign_out": "Выйти",
        "shell.theme.dark": "Темная",
        "shell.theme.light": "Светлая",
        "shell.theme.icon.dark": "ТМ",
        ...
        "page.events.subtitle": "Выполняйте запросы в стиле ClickHouse по events_view, переходите по источникам и учетным записям и открывайте детали события только когда это действительно нужно.",
        "page.alerts.title": "Инциденты и алерты",
        "page.alerts.subtitle": "Расследуйте агрегированные инциденты и очередь сырых алертов в том же side-drawer потоке, что и в консоли событий.",
        "page.assets.title": "Активы и контент детектирования",
        "page.assets.subtitle": "Управляйте устройствами, CMDB-контекстом, индикаторами TI, Sigma-детектами, Windows-коллекторами и контентом мониторинга.",
        "page.reports.title": "Отчеты и находки",
        "page.reports.subtitle": "Отчеты сканера уязвимостей, импортированные находки и переходы в Event Console для уязвимых активов и сервисов.",
        "page.resources.title": "Ресурсы платформы",
        "page.resources.subtitle": "Состояние хранилища, инвентарь правил, ретеншн и справка по запросам для активного развертывания SIEM.",
        "page.sources.title": "Инвентарь источников",
        "page.sources.subtitle": "Единое представление по источникам, типам телеметрии, collector path, владельцам из CMDB и актуальности данных.",
        ...
        "doc.quickstart": "Быстрый старт",
        "doc.quickstart.subtitle": "Используйте эти разделы как внутреннюю базу знаний для аналитиков.",
        "doc.quickstart.1": "Начинайте с дашбордов для общей картины и с инцидентов для активного триажа.",
        "doc.quickstart.2": "Используйте события для pivot-поиска, проверки raw-данных и восстановления таймлайна.",
        "doc.quickstart.3": "Используйте Источники и Коллекторы, чтобы сначала убедиться в здоровье телеметрии, и только потом делать выводы об отсутствии активности.",
        "doc.quickstart.4": "Используйте Активы для поддержки CMDB, TI, active lists, детектов и будущего контента нормализации.",
        "doc.playbooks": "Плейбуки расследований",
...
RU_INVESTIGATION_PLAYBOOKS: List[Dict[str, Any]] = [
    {
...
def investigation_playbooks(lang: str) -> List[Dict[str, Any]]:
    ...
    if lang == "ru":
        return RU_INVESTIGATION_PLAYBOOKS
    return playbooks["en"]
