...
function Chips({ values, empty = "Нет данных" }: { values: unknown; empty?: string }) {
  const items = Array.isArray(values) ? values.map((item) => text(item, "")).filter(Boolean) : [];
...
function TimelineTab({ detail }: { detail: IncidentDetailResponse }) {
  ...
  return <ol className="sentinel-incident-timeline">{timeline.map((item, index) => <li key={`${text(item.event_id ?? item.ts)}-${index}`}><i className={`tone-${severityTone(item.severity)}`} /><time>{formatTime(item.ts)}</time><div><strong>{text(item.description ?? item.type)}</strong><span>{text(item.source)} · {text(item.entity)}</span></div></li>)}</ol>;
}
...
function RuleTab({ detail }: { detail: IncidentDetailResponse }) {
  ...
  return <div className="sentinel-detail-stack">
    ...
    <section><h3>Аутентификация</h3><KeyValue rows={[["Всего событий", number(auth.auth_event_count)], ["Неуспешные входы", number(auth.failed_login_count)], ["Успешные входы", number(auth.successful_login_count)], ["Пользователи", text(auth.users)], ["Хосты", text(auth.hosts)]]} /></section>
  </div>;
}
...
export function IncidentDetailContent({ detail }: { detail: IncidentDetailResponse }) {
  ...
  return <div className="kuma-drawer sentinel-incident-detail"><Tabs label="Разделы карточки инцидента" onChange={setTab} value={tab} items={[
    { id: "summary", label: "Сводка" }, { id: "timeline", label: "Хронология", count: rows(detail.timeline).length }, { id: "evidence", label: "Evidence", count: number(related.total) + number(rawAlerts.total) }, { id: "rule", label: "Правило", count: rows(detail.rules).length }, { id: "json", label: "JSON" },
  ]} />
    ...
    {tab === "json" ? <pre className="sentinel-json">{JSON.stringify(detail.json_view ?? detail, null, 2)}</pre> : null}
  </div>;
}
...
export function EventDetailContent({ event }: { event: Row }) {
  ...
  return <div className="kuma-drawer sentinel-event-detail"><section className="sentinel-event-hero"><Badge tone={severityTone(event.severity)}>{text(event.severity)}</Badge><div><strong>{text(event.message, text(event.category))}</strong><span>{text(event.log_source)} · {formatTime(event.ts)}</span></div></section><Tabs label="Разделы события" onChange={setTab} value={tab} items={[{ id: "normalized", label: "Нормализованные поля" }, { id: "context", label: "Контекст" }, { id: "json", label: "JSON" }]} />
    ...
    {tab === "json" ? <pre className="sentinel-json">{JSON.stringify(event, null, 2)}</pre> : null}
  </div>;
}
