...
import type { DiscoveryJob, GeneratedReportRecord, ResponseExecutionRecord } from "./runtime/types";
...
type TaskKind = "report" | "response" | "discovery";
...
export async function loadTaskFeed(): Promise<TaskFeed> {
  const [reports, response, discovery] = await Promise.allSettled([
...
export function TaskDispatcherView({ navigate }: { navigate: (view: View) => void }) {
  ...
  const targetView: Record<TaskKind, View> = { report: "reports", response: "response", discovery: "discovery" };
  ...
  return <div className="native-page task-dispatcher-page">
    ...
    {state.data ? <>
      <section className="metric-grid">
        ...
        <div className="metric"><span>Контуры</span><strong>{new Set(all.map((item) => item.kind)).size}/3</strong><small>Reporting · SOAR · Discovery</small></div>
      </section>
