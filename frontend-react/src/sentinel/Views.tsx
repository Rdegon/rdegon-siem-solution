...
function JsonDetails({ value }: { value: unknown }) {
  return <pre className="sentinel-json">{JSON.stringify(value, null, 2)}</pre>;
}
...
export function SourcesView({ notify }: { notify: Notify }) {
  ...
  return <div className="native-page"><PageHeader title="Источники и коллекторы" actions={<IconButton icon="refresh" label="Обновить" onClick={state.reload} />} />
    ...
    <DetailDrawer onClose={() => setSelected(null)} open={Boolean(selected)} title={selected ? text(selected.source_name) : "Источник"}>{selected ? <div className="kuma-drawer"><section><h3>Паспорт источника</h3><KeyValue rows={[["Статус", <StatusCell key="s" value={text(selected.status)} />], ["Тип", text(selected.source_type)], ["IP-адреса", text(selected.source_ips ?? selected.observed_ips ?? selected.cmdb_ip)], ["Коллектор", text(selected.collector_name)], ["Asset ID", text(selected.cmdb_asset_id)], ["Владелец", text(selected.cmdb_owner)], ["Сервис", text(selected.cmdb_service)], ["Продукты", text(selected.products)], ["События", number(selected.events).toLocaleString("ru-RU")], ["Последнее событие", formatTime(selected.last_seen)]]} /></section><section><h3>Runtime record</h3><JsonDetails value={selected} /></section></div> : null}</DetailDrawer>
...
export function ResourcesView({ notify }: { notify: Notify }) {
  ...
  return <div className="native-page"><PageHeader title="Ресурсы платформы" actions={<><Button icon="plus" onClick={() => setEditing({ kind: "collector", tenant_id: "main", config: {}, bindings: {} })} tone="primary">Создать ресурс</Button><IconButton icon="refresh" label="Обновить" onClick={state.reload} /></>} />
    ...
    <DetailDrawer actions={selected ? <><Button icon="settings" onClick={() => setEditing(selected)}>Редактировать</Button><Button onClick={() => action("validate", selected)}>Проверить</Button><Button icon="play" onClick={() => action("publish", selected)} tone="primary">Опубликовать</Button></> : null} onClose={() => setSelected(null)} open={Boolean(selected)} title={selected ? text(selected.name) : "Ресурс"}>{selected ? <div className="kuma-drawer"><section><h3>Состояние</h3><KeyValue rows={[["ID", text(selected.id)], ["Тип", text(selected.kind)], ["Статус", <StatusCell key="status" value={text(selected.status)} />], ["Версия", text(selected.version)], ["Tenant", text(selected.tenant_id)], ["Опубликован", formatTime(selected.published_ts)]]} /></section><section><h3>Конфигурация</h3><JsonDetails value={{ config: selected.config, bindings: selected.bindings, activation: selected.activation }} /></section></div> : null}</DetailDrawer>
...
export function RulesView({ notify }: { notify: Notify }) {
  ...
  return <div className="native-page"><PageHeader title="Контент детектирования" actions={<><Button icon="plus" onClick={() => setEditing({ kind: "correlation", blocks: [] })} tone="primary">Создать черновик</Button><IconButton icon="refresh" label="Обновить" onClick={state.reload} /></>} />
    ...
    <DetailDrawer actions={selected ? tab === "drafts" ? <><Button icon="settings" onClick={() => setEditing(selected)}>Редактировать</Button><Button onClick={() => draftAction("validate", selected)}>Validate</Button><Button icon="play" onClick={() => draftAction("test", selected)}>Test</Button><Button onClick={() => draftAction("publish", selected)} tone="primary">Publish</Button></> : <><Button onClick={() => packAction("validate", selected)}>Validate</Button><Button icon="play" onClick={() => packAction("test", selected)}>Shadow test</Button><Button onClick={() => packAction("publish", selected)} tone="primary">Publish</Button></> : null} onClose={() => setSelected(null)} open={Boolean(selected)} title={selected ? text(selected.title) : "Правило"}>{selected ? <div className="kuma-drawer"><section><h3>Production object</h3><JsonDetails value={selected} /></section></div> : null}</DetailDrawer>
...
export function AssetsView() {
  ...
  ]} onOpen={setSelected} rows={rows} />}</QueryBoundary><DetailDrawer onClose={() => setSelected(null)} open={Boolean(selected)} title={selected ? text(selected.asset) : "Актив"}>{selected ? <div className="kuma-drawer"><section><h3>Карточка актива</h3><JsonDetails value={selected} /></section></div> : null}</DetailDrawer></div>;
}
...
export function ReportsView({ notify }: { notify: Notify }) {
  ...
  ]} onOpen={setSelected} rows={templates.items as Row[]} />}</>}</QueryBoundary><DetailDrawer actions={selected && tab === "templates" ? <Button icon="play" onClick={() => run(selected)} tone="primary">Сформировать сейчас</Button> : null} onClose={() => setSelected(null)} open={Boolean(selected)} title={selected ? text(selected.name) : "Отчет"}>{selected ? <JsonDetails value={selected} /> : null}</DetailDrawer></div>;
}
...
export function RuntimeView() {
  ...
  return <div className="native-page"><PageHeader title="Состояние платформы" actions={<IconButton icon="refresh" label="Обновить" onClick={state.reload} />} /><QueryBoundary state={state}>{({ platform, health, certification, hosts, ingest, dlq }) => <>
    ...
    ]} rows={hosts.targets as unknown as Row[]} /></section>
    <section className="panel"><header className="panel-header"><div className="panel-title"><h2>Ingest и health</h2><span>Сырые operational counters</span></div></header><JsonDetails value={{ ingest, health, certification }} /></section>
  </>}</QueryBoundary></div>;
}

export function AccessView({ notify }: { notify: Notify }) {
  const [tab, setTab] = useState("users"); const [selected, setSelected] = useState<Row | null>(null); const [creating, setCreating] = useState(false);
  const state = useQuery("access", async () => { const [users, accounts, grants, systems, providers] = await Promise.all([api.authUsers({ include_disabled: true }), api.serviceAccounts(), api.accessGrants({ include_disabled: true }), api.accessSystems(), api.authProviders()]); return { users, accounts, grants, systems, providers }; });
  async function createUser(form: HTMLFormElement) { const values = new FormData(form); try { await api.saveLocalUser({ username: text(values.get("username")), display_name: text(values.get("display_name"), ""), role: text(values.get("role"), "analyst"), enabled: true }); notify("Пользователь создан", "healthy"); setCreating(false); state.reload(); } catch (error) { notify(error instanceof Error ? error.message : String(error), "critical"); } }
  return <div className="native-page"><PageHeader title="Доступ и учетные записи" actions={<><Button icon="plus" onClick={() => setCreating(true)} tone="primary">Создать пользователя</Button><IconButton icon="refresh" label="Обновить" onClick={state.reload} /></>} /><QueryBoundary state={state}>{({ users, accounts, grants, systems, providers }) => { const tabs = [{ id: "users", label: "Пользователи", count: users.items.length }, { id: "accounts", label: "Service accounts", count: accounts.items.length }, { id: "grants", label: "Назначения", count: grants.items.length }, { id: "systems", label: "Системы", count: systems.items.length }, { id: "providers", label: "Провайдеры", count: providers.items.length }]; const rows = tab === "users" ? users.items : tab === "accounts" ? accounts.items : tab === "grants" ? grants.items : tab === "systems" ? systems.items : providers.items; return <><Tabs items={tabs} label="Управление доступом" onChange={setTab} value={tab} /><DataTable columns={[
    ...
    ]} onOpen={setSelected} rows={rows as Row[]} /></>; }}</QueryBoundary><DetailDrawer onClose={() => setSelected(null)} open={Boolean(selected)} title={selected ? text(selected.username ?? selected.name ?? selected.id) : "Доступ"}>{selected ? <JsonDetails value={selected} /> : null}</DetailDrawer><Modal footer={<><Button onClick={() => setCreating(false)}>Отмена</Button><Button form="user-form" tone="primary" type="submit">Создать</Button></>} onClose={() => setCreating(false)} open={creating} title="Новый локальный пользователь"><form className="kuma-form-grid" id="user-form" onSubmit={(event) => { event.preventDefault(); void createUser(event.currentTarget); }}><Field label="Username"><input name="username" required /></Field><Field label="Отображаемое имя"><input name="display_name" /></Field><Field label="Роль"><select name="role"><option value="viewer">Viewer</option><option value="analyst">Analyst</option><option value="admin">Admin</option></select></Field></form></Modal></div>;
}
...
export function CoverageView() {
  ...
  ]} onOpen={setSelected} rows={data.items as Row[]} /></>}</QueryBoundary><DetailDrawer onClose={() => setSelected(null)} open={Boolean(selected)} title={selected ? text(selected.title) : "Интеграция"}>{selected ? <JsonDetails value={selected} /> : null}</DetailDrawer></div>;
}
...
export function IdentityView() {
  ...
  ]} onOpen={setSelected} rows={rows as Row[]} /></>; }}</QueryBoundary><DetailDrawer onClose={() => setSelected(null)} open={Boolean(selected)} title={selected ? text(selected.username ?? selected.name ?? selected.client_id) : "Identity"}>{selected ? <JsonDetails value={selected} /> : null}</DetailDrawer></div>;
}
...
export function SecurityServiceView({ view, notify }: { view: View; notify: Notify }) {
  ...
  return <div className="native-page"><PageHeader title={view === "ngfw" ? "OPNsense Network Firewall" : view === "ids" ? "Suricata Network IPS" : view === "ndr" ? "Network Detection and Response" : view === "container" ? "Container Runtime Security" : view === "vpn" ? "VPN и защищенный доступ" : view === "dfir" ? "Endpoint DFIR" : view === "analysis" ? "Malware Analysis" : view === "evidence" ? "Evidence Storage" : "Internal PKI"} actions={<><IconButton icon="refresh" label="Обновить" onClick={state.reload} />{view === "ngfw" ? <Button icon="plus" onClick={() => setEditing({ interface: "opt5", action: "block", protocol: "any", source: "any", destination: "any", sequence: 100, enabled: false, log: true })} tone="primary">Создать правило</Button> : null}{view === "ids" ? <><Button onClick={async () => { try { await api.mutateIds("reload"); notify("Suricata policy перезагружена", "healthy"); state.reload(); } catch (error) { notify(error instanceof Error ? error.message : String(error), "critical"); } }}>Reload</Button><Button onClick={async () => { try { await api.mutateIds("update"); notify("Сигнатуры обновлены", "healthy"); state.reload(); } catch (error) { notify(error instanceof Error ? error.message : String(error), "critical"); } }} tone="primary">Обновить сигнатуры</Button></> : null}</>} />
    ...
    <DetailDrawer actions={selected?.address ? <a className="button button-primary" href={text(selected.address)} rel="noreferrer" target="_blank">Открыть систему</a> : null} onClose={() => setSelected(null)} open={Boolean(selected)} title={selected ? text(selected.title) : "Сервис"}>{selected ? <JsonDetails value={selected} /> : null}</DetailDrawer>
