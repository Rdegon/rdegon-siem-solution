...
SELECT
    ...
    'open' AS status
FROM siem.alerts_raw AS raw_alert
WHERE lower(raw_alert.status) = 'open'
...
class AlertAggWorker:
    ...
    def _run_aggregation(self) -> int:
        ...
        self._client.execute("TRUNCATE TABLE siem.alerts_agg SYNC")
        self._client.execute(AGG_INSERT_SQL)
        rows = self._client.execute("SELECT count() FROM siem.alerts_agg")
    ...
    async def run(self) -> None:
        ...
        while True:
            try:
                ...
                logger.info("Alert aggregation completed", extra={"extra": {"groups_count": groups_count}})
            except Exception as exc:  # noqa: BLE001
                logger.error("Alert aggregation failed", extra={"extra": {"error": str(exc)}})
