...
def test_assignment_overrides_replace_broad_fp_keywords_with_source_event_semantics() -> None:
    ...
    assert "eps-bench" in str(overrides["HB-012"]["sql_template"])
    assert "eps-benchmark" in str(overrides["HB-012"]["sql_template"])
    assert "toString(tags), 'benchmark'" in str(overrides["HB-012"]["sql_template"])
